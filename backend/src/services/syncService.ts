...
async function syncPersistedGuildByDiscordGuildId(discordGuildId: string) {
  const guild = await prisma.guild.findUnique({
    ...
    include: {
      ...
      requiredRoles: true,
      members: { where: { active: true }, orderBy: { displayName: 'asc' }, include: { roles: true } },
    },
  });
  ...
  const syncedGuild = await prisma.guild.findUnique({
    where: { id: guild.id },
    include: { members: { where: { active: true }, orderBy: { displayName: 'asc' }, include: { roles: true } } },
  });
  if (!syncedGuild) return true;
  ...
  publishGuildMembersDelta({
    ...
    upsertMembers: afterMembers.filter(member => beforeMap.get(member.id) !== JSON.stringify(member)),
    removedMemberIds: beforeMembers.filter(member => !afterMap.has(member.id)).map(member => member.id),
  });
...
async function runGuildSync(discordGuildId: string) {
  ...
  try {
    ...
    if (syncPendingGuilds.delete(discordGuildId)) queueGuildSync(discordGuildId, 0);
  }
}
...
export function queueGuildSync(discordGuildId: string, delayMs?: number) {
  ...
  const timer = setTimeout(() => {
    ...
    void runGuildSync(discordGuildId).catch(err => console.error(`[Discord Sync] Guild ${discordGuildId} failed:`, err?.message || err));
  }, Math.max(0, debounceMs));
  syncTimers.set(discordGuildId, timer);
}

export async function syncAllPersistedGuilds() {
  const guilds = await prisma.guild.findMany({ select: { discordGuildId: true } });
  guilds.forEach(guild => queueGuildSync(guild.discordGuildId, 0));
}
...
export async function syncActiveGuildMembers(userId: string, activeGuildId: string | null | undefined) {
  ...
  if (!access) return { status: 404 as const, body: { error: 'Chưa có server nào được import.' } };
  if (access.forbidden) return { status: 403 as const, body: { error: 'Bạn không có quyền đồng bộ thành viên.' } };
  const guild = await prisma.guild.findUnique({ where: { id: access.guild.id }, select: { discordGuildId: true } });
  if (!guild) return { status: 404 as const, body: { error: 'Chưa có server nào được import.' } };
  await runGuildSync(guild.discordGuildId);
  return { status: 200 as const, body: await getUserAppState(userId, activeGuildId) };
}
