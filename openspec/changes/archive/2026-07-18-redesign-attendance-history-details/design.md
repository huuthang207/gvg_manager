...
### 5. Normalize legacy MAYBE votes in the history UI
**Decision:** Frontend SHALL model raw persisted vote choices separately from current choices, then chỉ dùng `GO` và `NOGO` để tính và render lịch sử. Một vote `MAYBE` cũ sẽ không hiển thị như trạng thái phản hồi và thành viên tương ứng được coi là chưa phản hồi.

**Rationale:** Database và API phải tiếp tục đọc được record lịch sử, nhưng UI hiện hành không được tái tạo semantics dự bị hoặc crash khi nhận dữ liệu cũ. Normalization tại `SessionDetailsPanel` giữ scope ở frontend và không làm đổi contract backend.

**Alternatives considered:**
- Xóa `MAYBE` khỏi Prisma/database: phá khả năng đọc lịch sử và vượt scope UI.
- Hiển thị badge `MAYBE` trung tính: vẫn duy trì một trạng thái phản hồi đã bị loại bỏ.
- Ép kiểu raw value thành `GO`/`NOGO`: che giấu dữ liệu runtime không hợp lệ thay vì xử lý rõ ràng ở UI boundary.
